...
os.environ['CUDA_LAUNCH_BLOCKING'] = '1'  # 准确定位内存泄漏位置
...
args = {
    ...
    'load_path': os.path.join(working_path, 'checkpoints', DATA_NAME,'pre.pth'),
...
def main():
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_steps, eta_min=1e-7)
    warmup_scheduler = warmup.UntunedLinearWarmup(optimizer)

    train(train_loader, net, criterion, optimizer, scheduler, val_loader, warmup_scheduler)

    writer.close()
    print('Training finished.')


def train(train_loader, net, criterion, optimizer, scheduler, val_loader, warmup_scheduler):
    bestaccT = 0
    bestFscdV = 0.0
    bestloss = 1.0
